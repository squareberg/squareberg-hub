...
This copies the app into `apps/hello/`, creates `apps/hello/.venv`, installs the backend, and builds the active frontend.

!!! tip "Already have a copy?"
    If `apps/hello` already exists from a previous run, remove it first:
    ```bash
...
You should see the hello-world frontend. The hub is serving the pre-built static files from `apps/hello/frontend/default/dist/`.
...
|------|---------|-------------|
| Start hub | `sqb start` | Hub scans `apps/` and `examples/`, starts on port 9100 |
